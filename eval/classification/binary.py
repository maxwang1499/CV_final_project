...
def get_optimal_f1(groundtruth, probabilities,
    ...
    for idx in range(len(prec)):
        if (prec[idx] == 0) and (rec[idx] == 0):
            f1_values.append(0)
        else:
            f1_values.append(2 * (prec[idx] * rec[idx])/(prec[idx] + rec[idx]))
    f1_values = np.nan_to_num(f1_values)[:-1]
    # print('f1_values', f1_values)

    argmax_f1 = np.argmax(f1_values)
...
def get_binary_metrics(probs, labels, threshold=None, return_predictions=False):
